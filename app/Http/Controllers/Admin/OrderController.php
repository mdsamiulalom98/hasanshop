...
class OrderController extends Controller
{
    ...
    public function index($slug, Request $request)
    {
        if ($slug == 'all') {
            ...
            $show_data = Order::latest()->with('shipping', 'status')->where('order_type', 'website');
            if ($request->keyword) {
            ...
            $show_data = Order::where(['order_status' => $order_status->id, 'order_type'=> 'website'])->latest()->with('shipping', 'status')->paginate(50);
        }
        $users = User::get();
        ...
        //     $pathaostore = [];
        // }
        return view('backEnd.order.index', compact('show_data', 'order_status', 'users', 'steadfast'));
    }
    public function pos_orders(Request $request)
    {

        $show_data = Order::latest()->with('shipping', 'status')->where('order_type', 'pos');
        if ($request->keyword) {
            $show_data = $show_data->where(function ($query) use ($request) {
                $query->orWhere('invoice_id', 'LIKE', '%' . $request->keyword . '%')
                    ->orWhereHas('shipping', function ($subQuery) use ($request) {
    ...
    {
        $order = Order::where(['invoice_id' => $invoice_id])->with('orderdetails.image', 'orderdetails.warranty', 'payment', 'shipping', 'customer')->firstOrFail();
        return view('backEnd.order.invoice', compact('order'));
    }

    public function process($invoice_id)
    {
        $data = Order::where(['invoice_id' => $invoice_id])->select('id', 'invoice_id', 'order_status', 'order_type')->with('orderdetails.allimei', 'shipping')->first();
        // return $data;


        // return $orderDetail->imei;
    ...
    public function order_process(Request $request)
    {
        ...
        if ($request->status == 6 && $order_status != 6) {
            ...
            $proids = $request->ids;
            $imei = $request->imei;
            foreach ($proids as $key => $id) {
                if (isset($imei[$key])) {
                    $order_details = OrderDetails::find($id);
                    $order_details->imei = $imei;
                    $order_details->save();

                    $product = Product::select('id', 'imei')->find($order_details->product_id);
                    $product->imei = json_encode(array_values(array_diff(json_decode($product->imei, true), [$imei])));
                    $product->save();
                }
            }
            //return "wait";
            ...
            if ($order->affiliate_id != NULL) {
                ...
                $seller = Customer::find($order->affiliate_id);
                $seller->balance += $total_commision;
                $seller->earning += $total_commision;
                $seller->save();
            }
        }
    ...
    public function order_status(Request $request)
    {
        ...
        if ($request->order_status == 6) {
            ...
            $orderItems = Order::whereIn('id', $request->input('order_ids'))
                ->with(['orderdetails.product' => function ($query) {
                    $query->whereNotNull('imei');
                }])
                ->get();

            // Check for orders with products having non-null IMEI
            $hasNonNullIMEI = false;

            foreach ($orderItems as $order) {
                foreach ($order->orderdetails as $detail) {
                    if ($detail->product && $detail->product->imei !== null) {
                        $hasNonNullIMEI = true;
            ...
            if ($hasNonNullIMEI) {
                return response()->json(['status' => 'stop', 'message' => 'You got an emei product in your list']);
            }
    ...
    public function order_store(Request $request)
    {
        ...
        if ($request->pos_order == 1) {
            return redirect()->route('admin.order.invoice', ['invoice_id' => $order->invoice_id]);
        }
